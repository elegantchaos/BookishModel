...
class PublisherDetailTests: ModelTestCase {
    ...
    func testRowCount() {
        ...
        source.filter(for: ModelSelection([publisher]), editing: false, combining: false, session: TestContext())
        XCTAssertEqual(source.itemCount(for: 0), 3)
        XCTAssertEqual(source.itemCount(for: 1), 0)
        XCTAssertEqual(source.sectionCount, 2)

        source.filter(for: ModelSelection([publisher]), editing: false, combining: false, session: TestContext(showDebug: true))
